...
stop 工具会返回 `stop_result`：`stopped_now` 表示本次调用发送了停止/取消信号；`already_exited`、`already_completed`、`already_stopped` 表示调用前目标已结束，本次没有发送停止信号。模型应据此决定是读取 output/status，还是 cleanup。
...
`background_stop` 同样返回 `stop_result`，用于区分本次停止成功和任务已自然退出。
...
interactive_write(id="tty_1", input="partial", submit=false)
interactive_write(id="tty_1", input="", submit=true)
```

- 默认提交（追加 Enter），`submit=false` 明确支持无换行/分段输入，不会自动提交
- `input=""` 是合法输入；配合默认 `submit=true` 可发送空回车推进 prompt
- 普通文本使用 `interactive_write`；`ctrl-c`、`tab`、`esc` 等特殊键使用 `interactive_keys`
...
interactive_keys(id="tty_1", key="tab")
```

支持：`enter`、`ctrl-c`、`ctrl-d`、`tab`、`esc`。普通文本、分段输入和空回车优先使用 `interactive_write`：`submit=false` 用于无换行输入，`input="" submit=true` 用于空回车。
...
`agent_stop` 返回 `stop_result`：`stopped_now` 表示本次取消了运行中的 agent；`already_completed`、`already_failed`、`already_stopped` 表示调用前 agent 已结束或停止。

### 6.9 计划模式工具

```
plan_mode_enter(path="plans/arch.md", create_template=true)
plan_mode_exit()
plan_mode_status()
```

- `path` 会解析为绝对路径，并且必须位于当前 workspace 的 `plans/`、`.natalia/plans/` 或 `.kilo/plans/` 目录内
- `create_template=true` 只会在允许的 workspace plan 路径创建模板；workspace 外路径或非计划目录路径会返回错误且不会进入 Plan Mode
- `plan_mode_status()` 会显示计划文件是否存在、mtime、checklist 进度和下一条未完成项
...
workflow_run(name="review", action="status", state_path="/tmp/state.json")
workflow_run(name="review", action="complete_step", state_path="/tmp/state.json")
```

Workflow 文件位于 `.natalia/workflows/*.yaml`、`.natalia/commands/*.md`；`package.json` 中的 scripts 和 `Makefile` 中的 targets 也会自动导入。

`workflow_run(dry_run=true)` 只预览步骤和规范化内容，不初始化执行状态，适合审计、CI 和自动化验证。`dry_run=false` 是真实执行入口：它会创建或读取 workflow run state，并把当前步骤指令交给 agent 执行；后续步骤是否读写 workspace、运行命令或修改文件取决于 workflow 内容。内置 demo workflow 也按真实 workflow 语义处理，自动化测试应优先使用 `dry_run=true` 或专用无副作用测试 workflow。

真实执行状态机：

- `action="start"`（默认）：如果 `state_path` 已存在则恢复当前步骤，否则从 step 1 初始化
- `action="status"`：只查看 `current_step`、`total_steps`、`status` 和 `next_action`，不写 state
- `action="complete_step"`：当前步骤完成后推进到下一步；最后一步完成后状态变为 `completed`
- `action="restart"`：重置为 step 1，并覆盖 `state_path`
- 多步 workflow 应始终传同一个 `state_path`，否则进度不会跨调用持久化

### 6.11 Todo 工具

```
todo_set(items=["完成模块A", "修复bug", "写测试"])
todo_set(items=[{"content":"完成模块A","status":"in_progress","priority":4}, {"text":"写测试","done":false}])
todo_set(items=[])       # 显式清空任务；参数错误不会静默清空
todo_add(items=["重构"])
todo_done(index=2)     # 标记第 2 项完成（1-based）
todo_list()
```

- `items` 支持字符串数组或对象数组；对象字段可用 `content`/`text`、`status`、`done`、`notes`、`priority`、`id`
- 非空任务设置后，`todo_list` 应能看到同一任务列表；参数类型错误会返回错误并保留原列表
