...
### Capability groups (21 groups · 170 optional members)
...
| intelligence   | `constitutionRules` · `decisionRecords` · `recordDecision` · `evidenceRecords` · `recordValidation` · `completions` · `recordCompletion` · `driftFindings` · `evaluateDrift` · `acknowledgeDriftFinding` · `confirmedWorkspaceChanges` · `registeredTools` · `requestOverride` · `approveOverride` · `notices`                                                                                                                                                                                                                                                      |
...
### RPC route table (175 methods → members)
...
- Runtime event types (`RuntimeEventData` union): 115.
- view-store projections (`case` labels in `packages/hosts/view-store/src`): 118.
...
| `driftFindings`                     | `drift.findings`                     | `sessionID?`: string                                                                                                                                                                                                                                                                                                                                                                                                               | Array<{ findingID: string; severity: "advisory" | "warning" | "high"; confidence: number; originalObjective: string; currentActivity: string; evidence: string[]; status: | "open" | "explained" | "disputed" | "dismissed" | "corrected" | "detour_declared"; rationale?: string; contractVersion: number; ruleHits: Array<{ rule: string; confidence: number }>; planID?: string; }>                                                                |
