...
处理并行工作时，可以要求 agent 启动多个独立子代理并统一等待。Natalia 使用
`agent_wait` 协调，因此安静运行的任务不会产生状态轮询噪声。等待超时只表示调用者
停止等待；子代理会继续运行，之后仍可再次等待。

停止操作有保护。`agent_stop` 必须提供原因，普通请求不会中断活跃或最近仍有活动的
子代理。必须立即中断时可使用需要审批的 force stop，并会在生命周期审计中明确记录。
