...
### Capability groups (20 groups · 152 optional members)
...
| sessions       | `sessionList` · `sessionTouch` · `sessionRename` · `sessionPin` · `sessionDuplicate` · `sessionFork` · `sessionDelete` · `sessionNew` · `sessionArchive` · `sessionRestore` · `sessionExport` · `sessionAttach`                                                                                                                                                                                     |
| mcp            | `mcpCatalog` · `getMcpPrompt` · `readMcpResource` · `mcpServerAdd` · `mcpServerRemove`                                                                                                                                                                                                                                                                                                              |
| extensions     | `plugins` · `commandCatalog` · `commandExecute` · `capabilities` · `pluginUnload` · `pluginReload` · `toolFamilyReload` · `projectionContributions`                                                                                                                                                                                                                                                 |
| management     | `permissionList` · `permissionSave` · `permissionDelete`                                                                                                                                                                                                                                                                                                                                            |
| automation     | `taskOverview` · `flowOverview` · `documentCatalog` · `loadFlowDocument` · `loadTaskDocument` · `installExampleDocuments` · `saveFlowDocument` · `deleteFlowDocument` · `saveTaskDocument` · `deleteTaskDocument` · `taskSchedule` · `taskUnschedule` · `taskPermissionPreview` · `taskPermissionPreviewDocument` · `previewSystemdCalendar` · `permissionProfileUsage` · `decomposeFlowConditions` |
...
### RPC route table (157 methods → members)
...
| `task.preview-document`              | `taskPermissionPreviewDocument`     | automation       | read  |
| `task.load`                          | `loadTaskDocument`                  | automation       | read  |
| `flow.load`                          | `loadFlowDocument`                  | automation       | read  |
| `flow.install-examples`              | `installExampleDocuments`           | automation       | write |
| `task.preview-calendar`              | `previewSystemdCalendar`            | automation       | read  |
| `task.permission-usage`              | `permissionProfileUsage`            | automation       | read  |
| `flow.decompose-conditions`          | `decomposeFlowConditions`           | automation       | read  |

### Write surface (`RPC_WRITE_METHODS`, 56 methods; read-only credentials get `-32001 refused`)
...
- view-store projections (`case` labels in `packages/hosts/view-store/src`): 98.
...
| `taskSchedule`                      | `task.schedule`                      | `input`: { path: string; calendar: string; scope: "user" | "system"; executable?: string; cliEntry?: string; }                                                                                                                                                                                                                                                                                               | { path: string; taskID: string; timerUnit: string; scope: "user" | "system"; normalizedCalendar: string; next: string[]; commands: string[]; }                                                                                                                                                                                                                                                                                                        |
...
| `chat.turn.started`             | `id`: string, `messageID`: string, `startedAt`: number, `internal?`: boolean                                                                                                                                                                                                                                                                                                                                                                                                                                               | —                                                                                                                                                                                    |
| `chat.turn.phase`               | `id`: string, `messageID`: string, `phase`: "waiting" | "thinking" | "generating" | "using_tool", `toolName?`: string                                                                                                                                                                                                                                                                                                                                                                                                      | —                                                                                                                                                                                    |
| `chat.turn.finished`            | `id`: string, `messageID`: string, `stopReason`: "done" | "error" | "cancelled", `startedAt`: number, `endedAt`: number, `error?`: string                                                                                                                                                                                                                                                                                                                                                                                  | —                                                                                                                                                                                    |
...
| `chat.thinking.delta`           | `id`: string, `messageID`: string, `text`: string                                                                                                                                                                                                                                                                                                                                                                                                                                                                          | —                                                                                                                                                                                    |
| `chat.tool.used`                | `id`: string, `messageID`: string, `toolName`: string, `status`: string, `summary`: string, `result?`: string, `argumentsRaw?`: string, `startedAt?`: number, `endedAt?`: number, `at`: string                                                                                                                                                                                                                                                                                                                             | —                                                                                                                                                                                    |
...
| `collab.chat`                   | `id`: string, `threadID`: string, `from`: "live_chat" | "main_agent", `to`: "live_chat" | "main_agent", `text`: string, `replyToID?`: string, `round`: number, `expectsReply`: boolean, `at`: string                                                                                                                                                                                                                                                                                                                       | —                                                                                                                                                                                    |
