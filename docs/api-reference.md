...
`session`-scoped API members manage *session records* inside the one runtime,
...
**Authentication is per-deployment, default-deny.** A server configured with
credentials rejects requests without a token (or with a wrong one) with the
same `401` response, so the token cannot be probed. There is no built-in
identity flow; a deployment that wants one builds it outside the runtime.
...
const runtime = createRealRuntimeClient({
  ...
  provider: {                        // plug a real provider here
    provider: "scripted",
    model: "scripted",
    async *stream() {
...
the *capability records loaded into this runtime*, while availability lists
*which API members are implemented and reachable*. The names are close; the
facts are not.
...
- **Session records are created, archived and exported over RPC.**
  ...
  journal as `{ seq, event }` pairs. What RPC still cannot do is *activate* a
...
- The same stream is available over WebSocket at `/ws`.
...
| `workspaceFiles`                    | `workspace.files`                    | `input?`: { query?, type?, limit? }                                    | RuntimeWorkspaceFileEntry[]                                                                                                                                                                                                                                    |
| `workspaceSearch`                   | `workspace.search`                   | `input`: { query, include?, limit? }                                   | RuntimeWorkspaceMatch[]                                                                                                                                                                                                                                        |
| `workspaceList`                     | `workspace.list`                     | `input?`: { path?, offset?, limit? }                                   | RuntimeWorkspaceListPage                                                                                                                                                                                                                                       |
| `workspaceRead`                     | `workspace.read`                     | `input`: { path, offset?, limit? }                                     | RuntimeWorkspaceContent                                                                                                                                                                                                                                        |
| `workspaceGlob`                     | `workspace.glob`                     | `input`: { pattern, path?, limit? }                                    | RuntimeWorkspaceFileEntry[]                                                                                                                                                                                                                                    |
...
| `newSession`                        | `session.new`                        | `input?`: Parameters<NonNullable<RuntimeClient["sessionNew"]>>[0]      | Awaited<ReturnType<NonNullable<RuntimeClient["sessionNew"]>>>                                                                                                                                                                                                  |
| `archiveSession`                    | `session.archive`                    | `id`: string                                                           | Awaited<ReturnType<NonNullable<RuntimeClient["sessionArchive"]>>>                                                                                                                                                                                              |
| `exportSession`                     | `session.export`                     | `id`: string                                                           | Awaited<ReturnType<NonNullable<RuntimeClient["sessionExport"]>>>                                                                                                                                                                                               |
| `permissionList`                    | `permission.list`                    | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["permissionList"]>>>                                                                                                                                                                                              |
| `permissionSave`                    | `permission.save`                    | `input`: Parameters<NonNullable<RuntimeClient["permissionSave"]>>[0]   | Awaited<ReturnType<NonNullable<RuntimeClient["permissionSave"]>>>                                                                                                                                                                                              |
| `permissionDelete`                  | `permission.delete`                  | `name`: string                                                         | Awaited<ReturnType<NonNullable<RuntimeClient["permissionDelete"]>>>                                                                                                                                                                                            |
| `mcpServerAdd`                      | `mcp.server.add`                     | `input`: Parameters<NonNullable<RuntimeClient["mcpServerAdd"]>>[0]     | Awaited<ReturnType<NonNullable<RuntimeClient["mcpServerAdd"]>>>                                                                                                                                                                                                |
| `mcpServerRemove`                   | `mcp.server.remove`                  | `name`: string                                                         | Awaited<ReturnType<NonNullable<RuntimeClient["mcpServerRemove"]>>>                                                                                                                                                                                             |
| `createAgent`                       | `agent.create`                       | `input`: Parameters<NonNullable<RuntimeClient["agentCreate"]>>[0]      | Awaited<ReturnType<NonNullable<RuntimeClient["agentCreate"]>>>                                                                                                                                                                                                 |
| `updateAgent`                       | `agent.update`                       | `input`: Parameters<NonNullable<RuntimeClient["agentUpdate"]>>[0]      | Awaited<ReturnType<NonNullable<RuntimeClient["agentUpdate"]>>>                                                                                                                                                                                                 |
| `deleteAgent`                       | `agent.delete`                       | `name`: string                                                         | Awaited<ReturnType<NonNullable<RuntimeClient["agentDelete"]>>>                                                                                                                                                                                                 |
| `discoverProvider`                  | `provider.discover`                  | `input`: Parameters<NonNullable<RuntimeClient["providerDiscover"]>>[0… | Awaited<ReturnType<NonNullable<RuntimeClient["providerDiscover"]>>>                                                                                                                                                                                            |
| `addProvider`                       | `provider.add`                       | `input`: Parameters<NonNullable<RuntimeClient["providerAdd"]>>[0]      | Awaited<ReturnType<NonNullable<RuntimeClient["providerAdd"]>>>                                                                                                                                                                                                 |
| `removeProvider`                    | `provider.remove`                    | `name`: string                                                         | Awaited<ReturnType<NonNullable<RuntimeClient["providerRemove"]>>>                                                                                                                                                                                              |
| `unloadPlugin`                      | `plugin.unload`                      | `id`: string                                                           | Awaited<ReturnType<NonNullable<RuntimeClient["pluginUnload"]>>>                                                                                                                                                                                                |
| `reloadPlugin`                      | `plugin.reload`                      | `id`: string                                                           | Awaited<ReturnType<NonNullable<RuntimeClient["pluginReload"]>>>                                                                                                                                                                                                |
...
| `rollback`                          | `prompt`                             | `checkpointID`: string, `options?`: { dryRun? }                        | SubmittedTurn                                                                                                                                                                                                                                                  |
| `checkpointList`                    | `checkpoint.list`                    | —                                                                      | RuntimeCheckpoint[]                                                                                                                                                                                                                                            |
| `checkpointPreview`                 | `checkpoint.preview`                 | `id`: string                                                           | CheckpointPreview                                                                                                                                                                                                                                              |
| `checkpointRollback`                | `checkpoint.rollback`                | `input`: { id, dryRun? }                                               | CheckpointPreview                                                                                                                                                                                                                                              |
...
| `sandboxResourceOutput`             | `sandbox.resource.output`            | `input`: { id, resourceID, maxBytes? }                                 | string                                                                                                                                                                                                                                                         |
...
| `sandboxResourceStop`               | `sandbox.resource.stop`              | `input`: { id, resourceID }                                            | RuntimeSandboxResource                                                                                                                                                                                                                                         |
| `snapshot`                          | `snapshot`                           | —                                                                      | RuntimeEvent                                                                                                                                                                                                                                                   |
| `history`                           | `session.history`                    | `options?`: { after?, limit? }                                         | { events: Array<{ seq: number; event: RuntimeEvent }>; hasMore: boolean; }                                                                                                                                                                                     |
| `messages`                          | `session.messages`                   | `options?`: { limit?, order?, cursor? }                                | RuntimeMessagePage                                                                                                                                                                                                                                             |
...
| `nativeTerminalList`                | `nativeTerminal.list`                | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["nativeTerminalList"]>>>                                                                                                                                                                                          |
| `nativeTerminalRead`                | `nativeTerminal.read`                | `id`: string                                                           | Awaited<ReturnType<NonNullable<RuntimeClient["nativeTerminalRead"]>>>                                                                                                                                                                                          |
| `nativeTerminalStop`                | `nativeTerminal.stop`                | `id`: string                                                           | Awaited<ReturnType<NonNullable<RuntimeClient["nativeTerminalStop"]>>>                                                                                                                                                                                          |
| `nativeTerminalOpenHub`             | `nativeTerminal.openHub`             | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["nativeTerminalOpenHub"]>>>                                                                                                                                                                                       |
...
| `nativeTerminalStart`               | `nativeTerminal.start`               | `input`: Parameters<NonNullable<RuntimeClient["nativeTerminalStart"]>… | Awaited<ReturnType<NonNullable<RuntimeClient["nativeTerminalStart"]>>>                                                                                                                                                                                         |
| `nativeTerminalWrite`               | `nativeTerminal.write`               | `input`: Parameters<NonNullable<RuntimeClient["nativeTerminalWrite"]>… | Awaited<ReturnType<NonNullable<RuntimeClient["nativeTerminalWrite"]>>>                                                                                                                                                                                         |
| `nativeTerminalResize`              | `nativeTerminal.resize`              | `input`: Parameters<NonNullable<RuntimeClient["nativeTerminalResize"]… | Awaited<ReturnType<NonNullable<RuntimeClient["nativeTerminalResize"]>>>                                                                                                                                                                                        |
| `constitutionRules`                 | `constitution.rules`                 | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["constitutionRules"]>>>                                                                                                                                                                                           |
| `decisionRecords`                   | `decision.records`                   | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["decisionRecords"]>>>                                                                                                                                                                                             |
| `evidenceRecords`                   | `evidence.records`                   | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["evidenceRecords"]>>>                                                                                                                                                                                             |
| `driftFindings`                     | `drift.findings`                     | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["driftFindings"]>>>                                                                                                                                                                                               |
| `registeredTools`                   | `tools.registered`                   | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["registeredTools"]>>>                                                                                                                                                                                             |
| `capabilities`                      | `capabilities`                       | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["capabilities"]>>>                                                                                                                                                                                                |
| `sessionSnapshot`                   | `session.snapshot`                   | —                                                                      | Awaited<ReturnType<NonNullable<RuntimeClient["sessionSnapshot"]>>>                                                                                                                                                                                             |
| `deleteFlowDocument`                | `flow.delete`                        | `input`: { path }                                                      | { path: string; deleted: boolean; alreadyDeleted: boolean; }                                                                                                                                                                                                   |
| `updateConfig`                      | `config.update`                      | `input`: { patch }, `scope?`: "project" | "global";                    | { applied: boolean; reason?: string }                                                                                                                                                                                                                          |
| `taskPermissionPreview`             | `task.preview`                       | `input`: { path }                                                      | { taskID: string; displayName: string; permissionProfile: string; flowID: string; flowDisplayName: string; enabledModules: number; blocked: Array<{ moduleID: string; reason: string }>; conditionlessModules: string[]; problems: string[]; valid: boolean; } |
...
| `step.retry.exhausted`      | `id`: string, `operation`: StepRetryOperation, `step`: number, `attempts`: number, `maxAttempts`: number, `reason`: ErrorKind, `statusCode?`: number, `message`: string                                                                                                                                                                                                                                                                                                                                                    |
...
| `evidence.recorded`         | `id`: string, `taskID`: string, `objective`: string, `changeType`: "added" | "modified" | "deleted", `summary`: string, `result`: "passed" | "failed" | "skipped", `safeSummary`: string, `knownGaps?`: string[]                                                                                                                                                                                                                                                                                                           |
...
| `sandbox.diff`              | `id`: string, `path`: string, `oldPath?`: string, `mode?`: string                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Type                         | Fields                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `AgentSelectionOutcome`      | `selected?`: string, `reason?`: string                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| `ChannelCapabilityReport`    | `name`: string, `groups`: ChannelCapabilityGroup[]                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `CheckpointPreview`          | `checkpointID`: string, `safetyCheckpointID?`: string, `dryRun`: boolean, `path`: string, `oldPath?`: string, `mode?`: string, `targetJournalOffset`: number, `targetStep`: number, `targetTokens`: number, `compactionGeneration`: number, `resources`: CheckpointResourcePolicy[], `ignoredFiles`: number, `diskUsageBytes`: number, `complete`: boolean, `warnings`: string[]                                                                                                                         |
| `CheckpointResourcePolicy`   | `id`: string, `action`: "stop" | "preserve_dirty" | "cancel" | "invalidate" | "none", `summary`: string                                                                                                                                                                                                                                                                                                                                                                                                  |
| `ContributedCommand`         | `name`: string, `title`: string, `category?`: string                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `FlowOverview`               | `flows`: FlowRow[]                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `FlowRow`                    | `flowID`: string, `displayName`: string, `path`: string, `stages`: FlowStageRow[], `enabledStages`: number, `problems`: string[]                                                                                                                                                                                                                                                                                                                                                                         |
| `InteractiveResponseOutcome` | `reason?`: string                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `MCPCatalogSnapshot`         | `prompts`: MCPPromptCatalog[], `resources`: MCPResourceCatalog[]                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `MCPPromptCatalog`           | `server`: string, `name`: string, `description?`: string, `description?`: string                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `RuntimeCapabilityReport`    | `missingRequired`: RequiredRuntimeMember[], `missing`: string[]                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `RuntimeCheckpoint`          | `id`: string, `sequence`: number, `turnID?`: string, `stepID?`: string, `step`: number, `createdAt`: string, `complete`: boolean, `errors`: string[], `files`: number, `changes`: number, `tokenEstimate`: number, `diskUsageBytes`: number                                                                                                                                                                                                                                                              |
...
| `RuntimeProjectedMessage`    | `id`: string, `turnID`: string, `submitted`: SubmittedTurn, `rows`: RuntimeProjectedMessageRow[]                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `RuntimeSessionSummary`      | `id`: string, `title`: string, `createdAt`: string, `lastAccessedAt?`: string, `pinned`: boolean, `archived?`: boolean, `events`: number, `pendingInputs`: number, `cancelled`: boolean, `resumable`: boolean                                                                                                                                                                                                                                                                                            |
...
| `ScheduledTaskOverview`      | `tasks`: ScheduledTaskRow[]                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `ScheduledTaskRow`           | `taskID`: string, `displayName`: string, `path`: string, `permissionProfile`: string, `flowID`: string, `enabledModules`: number, `retry`: NataliaTaskDocument["retry"], `alertChannels`: string[], `issueTarget?`: string, `dataSource?`: string, `scope`: "user" | "system", `timerUnit?`: string, `nextRun?`: string, `generatedCalendar?`: string, `status`: string, `startedAt`: string, `endedAt?`: string, `skipReason?`: string, `consecutiveFailures`: number, `pendingAlertDeliveries`: number |
