...
### Capability groups (20 groups · 134 optional members)
...
### RPC route table (139 methods → members)
...
### Write surface (`RPC_WRITE_METHODS`, 54 methods; read-only credentials get `-32001 refused`)
...
- view-store projections (`case` labels in `packages/hosts/view-store/src`): 88.
