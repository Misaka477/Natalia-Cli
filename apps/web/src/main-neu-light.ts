...
console.log(`[startup] runtime client +${(performance.now() - startupStart).toFixed(1)}ms`);
...
console.log(`[startup] ui host +${(performance.now() - startupStart).toFixed(1)}ms`);
...
console.log(`[startup] plugin ui bundles +${(performance.now() - startupStart).toFixed(1)}ms`);

// Load the Neumorphism light UI plugin
await host.load(createNataliaNeuLightPlugin());
console.log(`[startup] main ui loaded +${(performance.now() - startupStart).toFixed(1)}ms`);
