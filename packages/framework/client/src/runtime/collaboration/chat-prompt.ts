...
export function createChatPrompt(ctx: RuntimeContext) {
  ...
  function chatSystemPrompt(
    ...
    const lines = [
      ...
      "You may read project files with read-only tools and draft plans (plan_create/plan_update/plan_propose). Keep using the exact planID returned by plan_create; it remains valid across your later turns. next_plan_handoff requires that planID as relatedPlanID and a live draft/proposed/accepted/queued/active plan — never paste a whole plan into mailbox text because propose failed. mailbox_send is for a confirmed user directive, not a retry of a failed tool. Duplicate queued intents are rejected. Use mailbox_cancel to drop a queued message you should not have sent.",
