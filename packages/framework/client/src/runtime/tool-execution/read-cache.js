...
exports.stableCacheKey = stableCacheKey;
exports.executeWithReadCache = executeWithReadCache;
...
function stableCacheKey(input) {
    ...
    if (Array.isArray(input))
        return "[".concat(input.map(stableCacheKey).join(","), "]");
    if (input && typeof input === "object") {
        var entries = Object.entries(input)
            ...
            return "".concat(JSON.stringify(key), ":").concat(stableCacheKey(value));
        });
        return "{".concat(entries.join(","), "}");
    }
    return (_a = JSON.stringify(input)) !== null && _a !== void 0 ? _a : "null";
}
function executeWithReadCache(input) {
    return __awaiter(this, void 0, void 0, function () {
        var kind;
        return __generator(this, function (_a) {
            kind = input.fabric === undefined
                ? undefined
                : rina_1.READ_CACHE_TOOL_KINDS[input.toolName];
            if (!input.fabric || !kind)
                return [2 /*return*/, input.execute()];
            return [2 /*return*/, input.fabric.compute(kind, stableCacheKey(input.parsed), input.execute)];
        });
    });
}
