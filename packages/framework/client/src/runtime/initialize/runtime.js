...
exports.createInitializeRuntime = createInitializeRuntime;
function createInitializeRuntime(ctx) {
    var deps = ctx.state.initialize;
    return __assign(__assign({}, deps), { resolveService: ctx.ports.resolveService, get workspaceRoot() {
        ...
        set agentRegistry(value) {
            if (value)
                ctx.ports.setAgentRegistry(value);
        },
        get selectedAgent() {
        ...
        }, publish: ctx.ports.publish, publishForSession: ctx.ports.publishForSession, ensureExecution: ctx.ports.ensureExecution, executionForTurn: ctx.ports.executionForTurn, extensionEnabled: ctx.ports.extensionEnabled, extensionToolPermission: ctx.ports.extensionToolPermission, pluginsController: ctx.ports.getPluginsController(), applyAgentPolicy: ctx.ports.applyAgentPolicy, applyAgentProvider: ctx.ports.applyAgentProvider, resolveContextStatusConfig: ctx.ports.resolveContextStatusConfig, contextWindowResolver: ctx.ports.getContextWindowResolver(), modelRefKeyForSelection: ctx.ports.modelRefKeyForSelection, tryParseToolArguments: ctx.ports.tryParseToolArguments, parseToolArguments: ctx.ports.parseToolArguments, validateToolParameters: ctx.ports.validateToolParameters, authorizeWorkspaceRead: ctx.ports.authorizeWorkspaceRead, authorizeSandboxMerge: ctx.ports.authorizeSandboxMerge, toolSettings: ctx.ports.toolSettings, redactToolOutput: ctx.ports.redactToolOutput, redactToolOutputEnabled: ctx.ports.redactToolOutputEnabled, teamBehavior: ctx.ports.teamBehavior, tools: ctx.ports.getTools(), isToolAllowed: ctx.ports.isToolAllowed, createToolPolicyLayer: ctx.ports.createToolPolicyLayer, effectiveMaxSteps: ctx.ports.effectiveMaxSteps, skillService: ctx.ports.skillService, requestNaviWake: ctx.ports.requestNaviWake, createCollabChatTool: ctx.ports.createCollabChatTool, initializeCheckpointController: ctx.ports.initializeCheckpointController, runtimeStatusSnapshot: ctx.ports.runtimeStatusSnapshot, scheduleRuntimeStatusSnapshot: ctx.ports.scheduleRuntimeStatusSnapshot });
}
