...
fn line_at<'a>(text: &'a str, line_number: u32) -> &'a str {
    if line_number == 0 {
        return "";
    }
    text.split('\n').nth(line_number as usize - 1).unwrap_or("")
}
...
unsafe fn build_structured_binary(
    ...
    for hunk in diff.hunks() {
        ...
        while old_i < before_start && new_i < after_start {
            let text = line_at(old_text, old_i + 1);
        ...
        for line in hunk.before.clone() {
            ...
            let text = line_at(old_text, line_no);
        ...
        for line in hunk.after.clone() {
            let line_no = line + 1;
            let idx = line_items.len() as u32;
            let text = line_at(new_text, line_no);
        ...
        for pair in 0..remove_indexes.len().min(add_indexes.len()) {
            ...
            let old_line_text = line_at(old_text, old_line_no);
            let new_line_text = line_at(new_text, new_line_no);
