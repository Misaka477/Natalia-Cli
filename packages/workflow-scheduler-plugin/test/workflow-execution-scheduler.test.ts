...
test("idempotency keys are isolated by workspace", async () => {
  const scheduler = new WorkflowExecutionScheduler({ globalConcurrency: 2 });
  const first = scheduler.schedule({
    idempotencyKey: "request-1",
    idempotencyFingerprint: "task-a",
    workspaceRoot: "/workspace/a",
  ...
  await expect(first.result).resolves.toBe("a");
  await expect(second.result).resolves.toBe("b");
});
