...
export class WorkflowExecutionScheduler {
  ...
  schedule<T>(input: {
    ...
    if (input.idempotencyKey) {
      const idempotencyKey = `${workspaceRoot}\u0000${input.idempotencyKey}`;
      ...
      cancel: (reason = "workflow execution cancelled") => {
        ...
        if (!execution.started) {
          ...
          if (
            ...
            this.idempotency.get(this.idempotencyKey(execution)) === execution
          )
            this.idempotency.delete(this.idempotencyKey(execution));
  ...
  private idempotencyKey(
    execution: Pick<
      ...
      "workspaceRoot" | "idempotencyKey"
    >,
  ) {
    return `${execution.workspaceRoot}\u0000${execution.idempotencyKey}`;
  }
