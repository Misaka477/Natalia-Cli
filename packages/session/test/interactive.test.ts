...
test("interactive projection retains only requests without durable replies", () => {
  const projection = projectInteractiveRequests([
    { type: "approval.request", id: "approval_open", title: "Write", preview: "a" },
    { type: "approval.request", id: "approval_closed", title: "Shell", preview: "b" },
  ...
  expect(projection.approvals.map((request) => request.id)).toEqual(["approval_open"]);
  expect(projection.questions.map((request) => request.id)).toEqual(["question_open"]);
});
