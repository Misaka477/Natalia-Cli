...
const target = "packages/framework/client/src/runtime/main.ts";

test("deleted built-in plugin classifications cannot be recreated", () => {
  expect(
    findForbiddenRepositoryPathViolation(
  ...
  expect(
    findRuntimePluginCatalogViolation(
      ...
      "computeBuiltinPluginGates(config)",
    ),
  ).toBeDefined();
  expect(
    findRuntimePluginCatalogViolation(
      "packages/framework/client/src/runtime/plugin-config/catalog.ts",
      "runtimePluginCatalog(config)",
    ),
  ).toBeUndefined();
});
...
test("client pure surfaces come from domain packages", () => {
  ...
  expect(
    findClientPluginSurfaceViolation(
      "packages/framework/client/src/runtime/composition/services.ts",
      'import { helper } from "@natalia/example-plugin"',
    ),
  ).toBe(
    "client runtime composition root must not import provider plugin packages",
  ...
  expect(
    findClientPluginSurfaceViolation(
      ...
      'import { TEAM_PLUGIN_ID } from "@natalia/plugin-team"',
    ),
  ).toBeDefined();
  expect(
    findClientPluginSurfaceViolation(
...
test("migrated plugin rules retain built-in plugin protections", () => {
  for (const [pluginID, source] of [
    ...
    ["natalia-subagents", 'import { agentTools } from "@natalia/tool-agent"'],
    [
    ...
    [
      "natalia-tool-terminal",
      'import { createTerminalPlugin } from "@natalia/plugin-native-terminal"',
    ],
    ["natalia-sandbox", 'import { sandboxTools } from "@natalia/tool-sandbox"'],
    [
    ...
    ["natalia-sandbox", "sandboxController?.get()"],
    ["natalia-sandbox", "new WorktreeSandboxManager(root)"],
    [
      "natalia-sandbox",
...
test("terminal migration rejects concrete native types in client composition", () => {
  for (const source of [
    'import type { NativeTerminalRegistry } from "@natalia/plugin-native-terminal"',
...
test("terminal migration rejects backend leaks across service consumers", () => {
  ...
  // The plugin package owns the native implementation; its own files may use
  // the concrete types and construction the client composition is barred from.
  expect(
    findMigratedPluginViolations(
      "packages/plugins/native-terminal/src/terminal-tools.ts",
      "function info(session: NativeTerminalSession) {}",
    ),
  ).toEqual([]);
  expect(
    findMigratedPluginViolations(
      "packages/plugins/native-terminal/src/index.ts",
      "export { NativeTerminalRegistry } from './native-terminal'",
    ),
  ).toEqual([]);
});
...
test("sandbox migration rejects backend leaks across service consumers", () => {
  for (const [path, source] of [
    ...
    ["packages/core/tools/src/types.ts", "sandboxes?: WorkspaceSandboxManager"],
    [
  ...
  expect(
    findMigratedPluginViolations(
      "packages/framework/client/src/runtime/composition/framework-services.ts",
      'import { createSandboxController } from "@natalia/sandbox"',
    ),
  ).toEqual([]);
});
...
test("subagents migration rejects backend leaks across service consumers", () => {
  for (const [path, source] of [
    ...
    ["packages/plugins/team/src/team-plugin.ts", "subagentsController?.get()"],
    [
      "packages/framework/subagents/src/tools.ts",
...
test("team migration rejects client-owned implementations", () => {
  ...
  expect(
    findMigratedPluginViolations(
      target,
      'import { createTeamPlugin } from "@natalia/plugin-team"',
    ),
  ).toContainEqual({
...
test("team plugin boundary keeps framework domains and client runtime off the team plugin", () => {
  for (const path of [
    "packages/framework/collaboration/src/interactive-waiter.ts",
    "packages/framework/collaboration/test/mailbox-ledger.test.ts",
    "packages/domains/work-ledger/src/work-ledger-controller.ts",
    "packages/domains/work-ledger/test/plan-ledger.test.ts",
    "packages/framework/client/src/runtime/main.ts",
    "packages/framework/client/src/runtime/plugin-assembly.ts",
  ])
    for (const source of [
      'import { TEAM_PLUGIN_ID } from "@natalia/plugin-team"',
      'import { runFanOut } from "../../team-plugin/src/fan-out"',
      'const create = require("@natalia/plugin-team")',
    ])
      expect(
        findTeamPluginDependencyViolation(path, source),
        `${path} ${source}`,
      ).toBeString();
});

test("team plugin boundary permits catalog imports and unrelated modules", () => {
  for (const path of [
    "packages/framework/client/src/runtime/plugin-config/catalog-entries.ts",
    "packages/framework/client/src/runtime/plugin-config/product-manifests.ts",
    "packages/framework/client/src/runtime/plugin-config/index.ts",
  ])
    expect(
      findTeamPluginDependencyViolation(
        path,
...
test("framework plugin-surface checks still cover collaboration and work-ledger", () => {
  for (const path of [
    "packages/framework/collaboration/src/interactive-waiter.ts",
    "packages/domains/work-ledger/src/work-ledger-controller.ts",
  ])
    for (const source of [
      ...
      "const TEAM_PLUGIN_ID = 'team'",
      "plugins.enabled[TEAM_PLUGIN_ID]",
      'import { x } from "@natalia/plugin"',
    ])
      expect(
        findFrameworkPluginSurfaceViolation(path, source),
        `${path} ${source}`,
      ).toMatch(/must not expose plugin lifecycle/u);
  expect(
    findFrameworkPluginSurfaceViolation(
      "packages/framework/collaboration/src/interactive-waiter.ts",
      "export const X = 1",
...
test("session store migration rejects client-owned implementations", () => {
  ...
  expect(
    findMigratedPluginViolations(
      "packages/framework/session-store/src/index.ts",
...
test("retry migration rejects client-owned retry implementations", () => {
  ...
  expect(
    findMigratedPluginViolations(
      target,
      'import type { RetryService } from "@natalia/retry"',
    ),
  ).toEqual([]);
});
...
test("attachment migration rejects client-owned implementations", () => {
  ...
  expect(
    findMigratedPluginViolations(
      target,
      'import type { AttachmentService } from "@natalia/attachments"',
    ),
  ).toEqual([]);
});
...
test("tool migrations protect the legacy static capability root", () => {
  for (const [pluginID, source] of [
    ...
    ["natalia-subagents", "agentToolFamily()"],
    ["natalia-tool-terminal", "terminalToolFamily()"],
    ["natalia-sandbox", "sandboxToolFamily()"],
...
test("only runtime plugin configuration may import concrete tool packages", () => {
  for (const [path, source] of [
    [
      "packages/framework/client/test/tool-catalogue.test.ts",
      'import { terminalTools } from "@natalia/tool-terminal"',
    ],
  ])
  ...
  expect(
    findClientToolDependencyViolation(
      "packages/framework/client/src/runtime/plugin-config/tool-catalog.ts",
      'import { createAskPlugin } from "@natalia/tool-ask"',
    ),
  ).toBeUndefined();
  expect(
    findClientToolDependencyViolation(
      "packages/framework/client/package.json",
      '"@natalia/tool-ask": "workspace:*"',
    ),
  ).toBeUndefined();
});
...
test("client physical dependency guard excludes extracted product packages", () => {
  for (const [path, source] of [
    ...
    [
      "packages/framework/client/test/mcp-controller.test.ts",
      'import type { MCPCatalogSnapshot } from "@natalia/mcp"',
    ],
    ["packages/framework/client/package.json", '"@natalia/mcp": "workspace:*"'],
    ["packages/framework/client/tsconfig.json", '"path": "../mcp"'],
  ])
  ...
  expect(
    findClientProductDependencyViolation(
      "packages/framework/client/tsconfig.json",
      '"path": "../mcp-plugin"',
    ),
  ).toBeUndefined();
  expect(
    findClientProductDependencyViolation(
      "packages/framework/client/src/fan-out.ts",
      'import type { SubagentRegistry } from "@natalia/subagent"',
    ),
  ).toBeString();
  expect(
  ...
  expect(
    findClientProductDependencyViolation(
      "packages/framework/client/src/terminal-controller.ts",
      'import { NativeTerminalRegistry } from "@natalia/plugin-native-terminal"',
    ),
  ).toBeString();
  expect(
    findClientProductDependencyViolation(
  ...
  expect(
    findClientProductDependencyViolation(
      "packages/framework/client/src/provider-runner.ts",
  ...
  expect(
    findClientProductDependencyViolation(
      "packages/framework/client/src/task-controller.ts",
  ...
  expect(
    findClientProductDependencyViolation(
      "packages/framework/client/package.json",
      '"@natalia/plugin-task-workflow": "workspace:*"',
    ),
  ).toBeUndefined();
  expect(
    findClientProductDependencyViolation(
...
test("client dependency closure permits ordinary plugins but rejects bundles", () => {
  expect(
    findClientClosureViolation(
      "packages/framework/client/package.json",
      '"@natalia/plugin-task-workflow": "workspace:*"',
  ...
  expect(
    findClientClosureViolation(
      "packages/framework/client/package.json",
      '"@natalia/plugin-tool-shell": "workspace:*"',
    ),
  ).toBeUndefined();
  expect(
    findClientClosureViolation(
  ...
  expect(
    findClientClosureViolation(
      "packages/framework/client/package.json",
      JSON.stringify({
        dependencies: { "@natalia/retry": "workspace:*" },
      }),
    ),
  ).toBeUndefined();
  expect(
    findClientClosureViolation(
      ...
      JSON.stringify({
        ...
        devDependencies: { "@natalia/retry": "workspace:*" },
      }),
    ),
  ).toBeUndefined();
  expect(
    findClientClosureViolation(
      "packages/framework/client/test/sandbox-controller.test.ts",
...
test("migrated plugin rules only protect declared composition roots", () => {
  ...
  expect(
    findMigratedPluginViolations(
      target,
      "runtimePluginCatalog(); entry.load()",
    ),
  ).toEqual([]);
  expect(
    findMigratedPluginViolations(
...
test("workflow scheduler migration protects the extracted implementation", () => {
  ...
  expect(
    findMigratedPluginViolations(
      "packages/framework/client/src/capability-execution-host.ts",
      'import type { WorkflowExecutionSchedulerService } from "@natalia/workflow-scheduler"',
    ),
  ).toEqual([]);

  for (const source of [
    'export { WorkflowExecutionScheduler } from "@natalia/workflow-scheduler"',
    'export { createWorkflowSchedulerHost } from "@natalia/workflow-scheduler"',
  ])
    expect(
      findMigratedPluginViolations(
        "packages/framework/client/src/index.ts",
  ...
  expect(
    findMigratedPluginViolations(
      "apps/cli/src/command-dispatcher.ts",
      'import { createWorkflowSchedulerHost } from "@natalia/client"',
    ),
  ).toContainEqual(
    expect.objectContaining({ pluginID: "natalia-workflow-scheduler" }),
  );
