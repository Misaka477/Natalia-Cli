...
var focusRe = /\b(?:test|it|describe)\.only\s*\(/u;
/** Intentional focus modifiers, keyed `path:line`, each with a reason. */
var ALLOWED = {};
function testFiles(dir) {
    return __asyncGenerator(this, arguments, function testFiles_1() {
            ...
            switch (_b.label) {
                ...
                case 5:
                    ...
                    if (skipDirs.has(entry.name))
                        return [3 /*break*/, 11];
                    return [5 /*yield**/, __values(__asyncDelegator(__asyncValues(testFiles((0, node_path_1.join)(dir, entry.name)))))];
                case 6: return [4 /*yield*/, __await.apply(void 0, [_b.sent()])];
                case 7:
                    _b.sent();
                    return [3 /*break*/, 11];
                case 8:
                    if (!/\.(test|spec)\.(ts|tsx)$/.test(entry.name)) return [3 /*break*/, 11];
...
var isCommentLine = function (line) { return /^\s*(?:\/\/|\/\*|\*)/.test(line); };
var failures = [];
var allowedPresent = new Set();
var scanned = 0;
for (var _i = 0, roots_1 = roots; _i < roots_1.length; _i++) {
    var base = roots_1[_i];
    try {
        for (var _d = true, _e = (e_1 = void 0, __asyncValues(testFiles((0, node_path_1.join)(root, base)))), _f; _f = await _e.next(), _a = _f.done, !_a; _d = true) {
            _c = _f.value;
            _d = false;
            var file = _c;
            scanned++;
            ...
            for (var _g = 0, _h = lines.entries(); _g < _h.length; _g++) {
                ...
                if (!focusRe.test(line) || isCommentLine(line))
                    continue;
                var key = "".concat(rel, ":").concat(index + 1);
                if (ALLOWED[key] !== undefined) {
                    allowedPresent.add(key);
                    continue;
                }
                failures.push("".concat(rel, ":").concat(index + 1, ": focused test (.only) is not allowed in committed tests"));
            }
        }
    }
...
for (var _k = 0, _l = Object.keys(ALLOWED); _k < _l.length; _k++) {
    var key = _l[_k];
    if (!allowedPresent.has(key))
        failures.push("".concat(key, ": ALLOWED entry is stale \u2014 the focus modifier is gone, remove it from the guard"));
}
console.log("test-focus guard: scanned ".concat(scanned, " test files"));
if (failures.length) {
    console.error("focused tests in committed test files: ".concat(failures.length));
    for (var _m = 0, failures_1 = failures; _m < failures_1.length; _m++) {
        var failure = failures_1[_m];
