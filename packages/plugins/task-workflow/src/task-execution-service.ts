...
export type TaskRuntimeClientOptions = {
  episodeID: EpisodeID;
  sessionID: SessionID;
  title: string;
...
export type TaskRuntimeClientFactory = (
  options: TaskRuntimeClientOptions,
) => RuntimeClient;
...
export async function taskPermissionPreviewForDocument(input: {
  ...
  const documents = createWorkflowStoreService(input);
...
export type { TaskRunResult } from "@natalia/runtime-services";
import type { TaskRunResult } from "@natalia/runtime-services";
...
export async function runTaskFromDocument(input: {
  ...
  const documents = createWorkflowStoreService({
    ...
    contributedDocuments: input.contributedDocuments,
  });
...
export async function runTask(input: {
  ...
  const state = await stores.openTaskState((event) => {
...
async function settleUnattendedState(input: {
  ...
  try {
    const store = await createWorkflowExecutionStoreService(
      input.workspaceRoot,
    ).openUnattendedState(input.invocation.taskID);
...
function taskIssueReporter(input: {
  ...
  return async (finding: {
    ...
    const state = await createWorkflowExecutionStoreService(
      input.workspaceRoot,
    ).openUnattendedState(input.task.taskID);
...
async function runTaskModule(input: {
  ...
  execution: import("@natalia/workflow").HeadlessExecution;
  module: NataliaPlannedFlowModule;
  reportIssue?: NonNullable<TaskModuleContext>["reportIssue"];
  readDataSource?: NonNullable<TaskModuleContext>["readDataSource"];
  json: boolean;
  emit: (line: string) => void;
  signal?: AbortSignal;
  createRuntimeClient: TaskRuntimeClientFactory;
  ...
  const client = input.createRuntimeClient({
    ...input.execution,
    workspaceRoot: input.workspaceRoot,
