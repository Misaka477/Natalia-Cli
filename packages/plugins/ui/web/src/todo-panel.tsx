          ...
          <div class="agent-empty-full">
            <div class="review-empty-icon">
              <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
                <rect x="6" y="6" width="24" height="24" rx="4" stroke="currentColor" stroke-width="1.6" />
                <path d="M12 18L16 22L24 14" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" />
              </svg>
            </div>
          ...
          <div class="review-diff">
            ...
            <div class="review-diff-content">
              <For each={visible()}>
                {(todo) => (
                  <div class="todo-row" data-status={todo.status}>
                    ...
                    <span class="todo-status">{todo.status === "in_progress" ? "进行中" : "待处理"}</span>
                  </div>
                )}
              </For>
              ...
              <Show when={completed().length}>
                <div class="todo-completed-header">已完成（{completed().length}）</div>
                <For each={completed()}>
                  {(todo) => (
                    <div class="todo-row" data-status="completed">
                      <span class="todo-dot" data-status="completed" />
                      <span class="todo-text todo-completed-text">{todo.content}</span>
                    </div>
                  )}
                </For>
              </Show>
