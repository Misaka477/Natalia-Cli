...
export function WorkspaceSettingsPanel(props: {
  ...
  onMount(() => {
    ...
    void props.runtime?.workspacePermissionGet?.(props.workspaceID).then((value) => {
  ...
  function save() {
    ...
    void props.runtime.workspacePermissionSet?.(props.workspaceID, permission());
    void props.runtime.workspaceToolSet?.(props.workspaceID, {
      enabledTools: enabledText().split("\n").map((line) => line.trim()).filter(Boolean),
      disabledTools: disabledText().split("\n").map((line) => line.trim()).filter(Boolean),
    });
    props.onClose();
  }
      ...
      <div class="neu-settings-backdrop" onClick={props.onClose}>
        <div class="neu-workspace-window" onClick={(event) => event.stopPropagation()}>
          <div class="neu-settings-header">
            ...
            <button type="button" class="neu-settings-close" onClick={props.onClose} aria-label="关闭工作区设置">
          ...
          <div class="neu-workspace-body">
            <div class="neu-form-field">
              ...
              <input class="neu-form-input" value={permission().permissionProfile} onInput={(event) => setPermission({ ...permission(), permissionProfile: event.currentTarget.value })} />
            </div>
            <div class="neu-form-field">
            ...
            <div class="neu-form-field">
              ...
              <textarea class="neu-form-input neu-stash-textarea" value={enabledText()} onInput={(event) => setEnabledText(event.currentTarget.value)} />
            </div>
            <div class="neu-form-field">
              <label class="neu-form-label">禁用工具（每行一个）</label>
              <textarea class="neu-form-input neu-stash-textarea" value={disabledText()} onInput={(event) => setDisabledText(event.currentTarget.value)} />
            </div>
