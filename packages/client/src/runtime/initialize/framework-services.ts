...
import { createSubagentsController } from "@natalia/subagents";
import { createTerminalController } from "@natalia/terminal";
...
export async function wireFrameworkServices(
  ...
  const terminalOwner = registry.registerOwner({
    id: "natalia-terminal",
    name: "Terminal",
    version: "1.0.0",
    scope: "session",
  ...
  const terminal = createTerminalController({
    ...
    external: options.nativeTerminal as Parameters<
      typeof createTerminalController
    >[0]["external"],
  });
  terminalOwner.contribute("services", TERMINAL_CONTROLLER_SERVICE, terminal);
  closeHandles.push(() => void terminal.close());
