...
test("runs a real command through an operating-system pseudo terminal", async () => {
  ...
  expect(result.state.transcript).toContain("terminal-ok");
...
test("persistent terminal registry records transcript and attach state", async () => {
  const root = await mkdtemp(join(tmpdir(), "natalia-terminal-persist-"));
  const registry = new PersistentTerminalRegistry(
    join(root, ".natalia", "terminal"),
  );
  const started = await registry.start({
    id: "tty_persist",
    command: "printf 'terminal-persist\\n'",
    cwd: root,
  });
  ...
  expect(await registry.transcript("tty_persist")).toContain(
    "terminal-persist",
  );
  ...
  const reopened = new PersistentTerminalRegistry(
    join(root, ".natalia", "terminal"),
  );
  expect(
...
test("interactive terminal registry writes input, special keys, resize and transcript", async () => {
  const root = await mkdtemp(join(tmpdir(), "natalia-terminal-interactive-"));
...
test("interactive terminal subscriptions omit full transcript from live updates", async () => {
  const root = await mkdtemp(join(tmpdir(), "natalia-terminal-light-update-"));
  const registry = new TerminalRegistry(join(root, ".natalia", "terminal"));
...
test("interactive Terminal sensitive input is redacted and audited", async () => {
  const root = await mkdtemp(join(tmpdir(), "natalia-terminal-secret-"));
  const registry = new TerminalRegistry(join(root, ".natalia", "terminal"));
  const started = await registry.start({ command: "cat", cwd: root });
...
test("interactive terminal projects ANSI style through the xterm framebuffer", async () => {
  const root = await mkdtemp(join(tmpdir(), "natalia-terminal-screen-"));
  const registry = new TerminalRegistry(join(root, ".natalia", "terminal"));
  const started = await registry.start({
