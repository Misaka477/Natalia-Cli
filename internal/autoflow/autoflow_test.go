...
func TestEscalatorSwitchesToDebugAfterThreshold(t *testing.T) {
	...
	decision := escalator.Record(&orchestrator.Outcome{StopReason: "error", FinalMessage: "first"}, "code")
	if decision.Action != ActionNone {
		t.Fatalf("expected no action on first error, got %+v", decision)
	}
	decision = escalator.Record(&orchestrator.Outcome{StopReason: "error", FinalMessage: "second"}, "code")
...
func TestEscalatorRecoversPreviousModeAfterDebugSuccess(t *testing.T) {
	...
	decision := escalator.Record(&orchestrator.Outcome{StopReason: "error", FinalMessage: "fail"}, "code")
	if decision.Action != ActionDebug {
		t.Fatalf("expected debug escalation, got %+v", decision)
	}
	decision = escalator.Record(&orchestrator.Outcome{StopReason: "no_tool_calls", FinalMessage: "fixed"}, "debug")
...
func TestEscalatorResetsAfterSuccess(t *testing.T) {
	...
	escalator.Record(&orchestrator.Outcome{StopReason: "no_tool_calls", FinalMessage: "ok"}, "code")
...
func TestEscalatorDoesNotEscalateAlreadyDebug(t *testing.T) {
	escalator := &Escalator{Threshold: 1}
	decision := escalator.Record(&orchestrator.Outcome{StopReason: "error"}, "debug")
	if decision.Action != ActionNone {
...
func TestEscalatorEscalatesOnMaxSteps(t *testing.T) {
	escalator := &Escalator{Threshold: 1}
	decision := escalator.Record(&orchestrator.Outcome{StopReason: "max_steps", FinalMessage: "达到最大步骤数"}, "code")
...
func TestClassifyFailure(t *testing.T) {
	cases := []struct {
		...
		{name: "error", out: &orchestrator.Outcome{StopReason: "error"}, want: FailureError},
		{name: "max steps", out: &orchestrator.Outcome{StopReason: "max_steps"}, want: FailureMaxSteps},
		{name: "success", out: &orchestrator.Outcome{StopReason: "no_tool_calls", FinalMessage: "ok"}, want: FailureNone},
		{name: "approval", out: &orchestrator.Outcome{StopReason: "requires_approval"}, want: FailureNone},
		{name: "transient flag", out: &orchestrator.Outcome{StopReason: "error", FinalMessage: "API error 400", Transient: true}, want: FailureTransient},
		{name: "transient msg", out: &orchestrator.Outcome{StopReason: "error", FinalMessage: "API error 502: upstream down", Transient: false}, want: FailureTransient},
		{name: "transient empty", out: &orchestrator.Outcome{StopReason: "error", FinalMessage: "empty assistant response from model"}, want: FailureTransient},
	}
...
func TestTransientFailuresDoNotIncrementConsecutive(t *testing.T) {
	...
	escalator.Record(&orchestrator.Outcome{StopReason: "error", FinalMessage: "API error 502", Transient: true}, "code")
	if escalator.Consecutive != 0 {
	...
	decision := escalator.Record(&orchestrator.Outcome{StopReason: "error", FinalMessage: "tool not found: foo"}, "code")
	...
	decision = escalator.Record(&orchestrator.Outcome{StopReason: "error", FinalMessage: "tool not found: bar"}, "code")
	if decision.Action != ActionDebug {
...
func TestEscalatorNilAndDefaultBoundaryBehavior(t *testing.T) {
	...
	if decision := nilEscalator.Record(&orchestrator.Outcome{StopReason: "error"}, "code"); decision.Action != ActionNone {
	...
	if decision := escalator.Record(&orchestrator.Outcome{StopReason: "error"}, ""); decision.Action != ActionNone {
		t.Fatalf("default threshold first failure should not escalate, got %+v", decision)
	}
	decision := escalator.Record(&orchestrator.Outcome{StopReason: "error"}, "")
...
func TestSuccessfulOutcomeRequiresFinalMessage(t *testing.T) {
	if isSuccessfulOutcome(&orchestrator.Outcome{StopReason: "no_tool_calls"}) {
		t.Fatal("expected no_tool_calls without final message to be unsuccessful")
	}
	if !isSuccessfulOutcome(&orchestrator.Outcome{StopReason: "no_tool_calls", FinalMessage: "done"}) {
