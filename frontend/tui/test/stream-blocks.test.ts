...
import { splitMarkdownAtSafeBoundary } from "../src/stream/markdown";
import { parseToolArguments, resultView } from "../src/tools/display";
...
test("partial tool arguments are hidden until complete and sensitive keys redact", () => {
  ...
  const parsed = parseToolArguments(
    JSON.stringify({ path: "frontend/tui", token: "secret", limit: 5 }),
  );
  expect(parsed.complete).toBe(true);
  expect(parsed.redactedJson).toContain("[REDACTED]");
  expect(parsed.keyArguments).toContain("path=frontend/tui");
  ...
  state = reduceState(state, {
    ...
    summary: "receiving",
    argumentsDelta: '{"path":"frontend/tui",',
  });
